...
class SpreadDemo {
    constructor(timeframe) {
        ...
        const orderbooks = this.service.spreadOrderbooks;
    ...
    updateDisplay() {
        ...
        // Header
        console.log(cyan('════════════════════════════════════════════════════════════════════════════════'));
        console.log(cyan(`                        SPREAD ORDERBOOK - ${this.timeframe}ms                        `));
        console.log(cyan('════════════════════════════════════════════════════════════════════════════════'));
        ...
        console.log(`Current Price: ${bold(priceColor(`$${this.currentPrice.toFixed(2)} ${priceChange >= 0 ? '↑' : '↓'}`))}  |  Time: ${new Date().toLocaleTimeString()}`);
        // Mini price chart
    ...
    renderSpreadTable() {
        ...
        const contracts = this.orderbook.contracts;
        ...
        // Current price line
        console.log(yellow(`══════╪${'═'.repeat(visibleColumns * 9 - 1)} Price: ${this.currentPrice.toFixed(2)}`));
    ...
    formatSpreadCell(contract, isCurrentColumn, isCall) {
    ...
    simulateTrading() {
        setInterval(() => {
            ...
            if (contracts.length > 0) {
                ...
                const priceDirection = this.priceHistory.length > 1
                    ? this.currentPrice -
                ...
                const filteredContracts = contracts.filter((c) => {
                    if (Math.random() < 0.3)
                        return true; // 30% random
                    if (priceDirection > 0 && c.spreadType === types_1.SpreadType.CALL)
                        return true;
                    if (priceDirection < 0 && c.spreadType === types_1.SpreadType.PUT)
                        return true;
                    return false;
                });
                if (filteredContracts.length > 0) {
                    const contract = filteredContracts[Math.floor(Math.random() * filteredContracts.length)];
                    const amount = Math.min(10 + Math.floor(Math.random() * 90), balance * 0.15);
                    try {
                        const result = this.service.placeSpreadPosition(user, contract.id, amount, this.timeframe);
                        if (result.success) {
